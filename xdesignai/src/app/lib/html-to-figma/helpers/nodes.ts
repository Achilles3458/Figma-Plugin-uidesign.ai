...
export function textNodesUnder(myDocument: Document, el: Element) {
  let n: Node | null = null;
  const a: Node[] = [];
  const walk = myDocument.createTreeWalker(el, NodeFilter.SHOW_TEXT, null);
...
export function isHidden(myWindow: Window, element: Element) {
...
export function getParents(node: Element | Node): Element[] {
  let el: Element | null =
    node instanceof Node && node.nodeType === Node.TEXT_NODE ? node.parentElement : (node as Element);
