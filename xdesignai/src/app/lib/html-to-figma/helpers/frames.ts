...
const getParent = ({ layer, root }: { layer: LayerNode; root: WithRef<FrameNode> }) => {
  let response: LayerNode | null = null;
  try {
    traverse(root, (child) => {
      if (child && (child as any).children && (child as any).children.includes(layer)) {
        response = child;
...
const makeTree = (
  ...
  while (updated) {
    ...
    traverse(root, (layer, originalParent) => {
      ...
      const node = layer.ref;
      let parentElement: Element | null = (node && (node as Element).parentElement) || null;
      do {
        if (parentElement === myWindow.document.body) {
          break;
        }
        if (parentElement) {
  ...
  // Collect tree of deepest common parents and make groups
    ...
    traverse(root, (layer, _) => {
      ...
      if (layer.type === 'FRAME') {
        ...
        if (layer.children && layer.children.length > 2) {
          const childRefs = layer.children && (layer.children as LayerNode[]).map((child) => child.ref!);
          ...
          for (const childRef of childRefs) {
            ...
            for (const otherChildRef of otherChildRefs) {
              ...
              for (const parent of otherParents) {
                if (childParents.includes(parent) && layer.ref!.contains(parent)) {
          ...
          if (lowestCommonDenominator && lowestCommonDenominator !== layer.ref) {
            ...
            const newChildren = layer.children!.filter((item: any) => lowestCommonDenominator.contains(item.ref));

            if (newChildren.length !== layer.children.length) {
              const lcdRect = getBoundingClientRect(myWindow, lowestCommonDenominator as Element);

              const overflowHidden =
                lowestCommonDenominator instanceof Element &&
                getComputedStyle(lowestCommonDenominator).overflow !== 'visible';
...
export const getLayersForFrames = (
  ...
  makeTree(myWindow, { root, layers });
  const framesLayers = [root];
  addConstraints(myWindow, framesLayers);
