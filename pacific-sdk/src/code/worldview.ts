...
function extractWorldviewPaintFromFills(
  sceneNode: SceneNode
): Transaction<WorldviewPaint | undefined> {
...
function extractWorldviewSolidPaintFromFills(
  sceneNode: SceneNode
...
function extractWorldviewSolidPaintFromStrokes(
  sceneNode: SceneNode
): Transaction<WorldviewSolidPaint | undefined> {
  ...
  if (!fillOK) {
    ...
    if ('children' in sceneNode) {
      for (const child of sceneNode.children) {
        const [childFill, childFillOK] =
          recurisvelyExtractWorldviewSolidPaintFromFills(child);
        if (!childFillOK) {
          return [null, false];
...
function recurisvelyExtractWorldviewSolidPaintFromStrokes(
  ...
  if (!strokeOK) {
    ...
    if ('children' in sceneNode) {
      for (const child of sceneNode.children) {
        const [childStroke, childStrokeOK] =
          recurisvelyExtractWorldviewSolidPaintFromStrokes(child);
        if (!childStrokeOK) {
          return [null, false];
...
export async function extractWorldviewContainerNode(
  ...
  WorldviewContainerNode | WorldviewComponentNode | WorldviewInstanceNode | null
> {
  ...
  } as WorldviewContainerNode | WorldviewComponentNode | WorldviewInstanceNode;
  ...
  if (frameNode.paddingBottom > 0)
    idealized.paddingBottom = frameNode.paddingBottom;
  if (frameNode.paddingLeft > 0) idealized.paddingLeft = frameNode.paddingLeft;
  if (frameNode.paddingRight > 0)
    idealized.paddingRight = frameNode.paddingRight;
  ...
  if (
    ...
    if (frameNode.strokeTopWeight > 0)
      idealized.strokeTopWeight = frameNode.strokeTopWeight;
    if (frameNode.strokeBottomWeight > 0)
      idealized.strokeBottomWeight = frameNode.strokeBottomWeight;
    if (frameNode.strokeLeftWeight > 0)
      idealized.strokeLeftWeight = frameNode.strokeLeftWeight;
    if (frameNode.strokeRightWeight > 0)
      idealized.strokeRightWeight = frameNode.strokeRightWeight;
    idealized.stroke = stroke;
  }

  // Corner radius
  if (frameNode.topLeftRadius > 0)
    idealized.topLeftRadius = frameNode.topLeftRadius;
  if (frameNode.topRightRadius > 0)
    idealized.topRightRadius = frameNode.topRightRadius;
  if (frameNode.bottomLeftRadius > 0)
    idealized.bottomLeftRadius = frameNode.bottomLeftRadius;
  if (frameNode.bottomRightRadius > 0)
    idealized.bottomRightRadius = frameNode.bottomRightRadius;
...
export async function extractWorldviewLineNode(
  node: LineNode
): Promise<WorldviewLineNode | null> {
  ...
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(lineNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (lineNode.strokeWeight === figma.mixed) {
    ...
    if (lineNode.strokeWeight > 0)
      idealized.strokeWeight = lineNode.strokeWeight;
  }
...
export async function extractWorldviewVoidContainerNode(
  ...
  // Stroke
  const [stroke, strokeOK] =
    extractWorldviewSolidPaintFromStrokes(rectangleNode);
  if (!strokeOK) {
    return null;
  }
  if (
    ...
    if (rectangleNode.strokeTopWeight > 0)
      idealized.strokeTopWeight = rectangleNode.strokeTopWeight;
    if (rectangleNode.strokeBottomWeight > 0)
      idealized.strokeBottomWeight = rectangleNode.strokeBottomWeight;
    if (rectangleNode.strokeLeftWeight > 0)
      idealized.strokeLeftWeight = rectangleNode.strokeLeftWeight;
    if (rectangleNode.strokeRightWeight > 0)
      idealized.strokeRightWeight = rectangleNode.strokeRightWeight;
    idealized.stroke = stroke;
  }

  // Corner radius
  if (rectangleNode.topLeftRadius > 0)
    idealized.topLeftRadius = rectangleNode.topLeftRadius;
  if (rectangleNode.topRightRadius > 0)
    idealized.topRightRadius = rectangleNode.topRightRadius;
  if (rectangleNode.bottomLeftRadius > 0)
    idealized.bottomLeftRadius = rectangleNode.bottomLeftRadius;
  if (rectangleNode.bottomRightRadius > 0)
    idealized.bottomRightRadius = rectangleNode.bottomRightRadius;
...
export async function extractWorldviewIconContainerNode(
  ...
  // Fill
  const [fill, fillOK] =
    recurisvelyExtractWorldviewSolidPaintFromFills(frameNode);
  if (!fillOK) {
    return null;
  }
  idealized.fill = fill;
  ...
  // Stroke
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(frameNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (frameNode.strokeWeight !== figma.mixed) {
    if (frameNode.strokeWeight > 0)
      idealized.strokeWeight = frameNode.strokeWeight;
  }
...
export async function extractWorldviewEllipseNode(
  node: EllipseNode
): Promise<WorldviewEllipseNode | null> {
  ...
  // Stroke
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(ellipseNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (ellipseNode.strokeWeight === figma.mixed) {
    ...
    if (ellipseNode.strokeWeight > 0)
      idealized.strokeWeight = ellipseNode.strokeWeight;
  }
...
export async function extractWorldviewStarNode(
  node: StarNode
): Promise<WorldviewStarNode | null> {
  ...
  // Stroke
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(starNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (starNode.strokeWeight === figma.mixed) {
    ...
    if (starNode.strokeWeight > 0)
      idealized.strokeWeight = starNode.strokeWeight;
  }
...
export async function extractWorldviewPolygonNode(
  node: PolygonNode
): Promise<WorldviewPolygonNode | null> {
  ...
  // Stroke
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(polygonNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (polygonNode.strokeWeight === figma.mixed) {
    ...
    if (polygonNode.strokeWeight > 0)
      idealized.strokeWeight = polygonNode.strokeWeight;
  }
...
export async function extractWorldviewTextNode(
  textNode: TextNode
): Promise<WorldviewTextNode | null> {
...
export async function extractWorldviewVectorNode(
  vectorNode: VectorNode
): Promise<WorldviewVectorNode | null> {
  ...
  // Fill
  const [fill, fillOK] =
    recurisvelyExtractWorldviewSolidPaintFromFills(vectorNode);
  if (!fillOK) {
    return null;
  }
  idealized.fill = fill;
  ...
  // Stroke
  const [stroke, strokeOK] =
    recurisvelyExtractWorldviewSolidPaintFromStrokes(vectorNode);
  if (!strokeOK) {
    return null;
  }
  idealized.stroke = stroke;
  ...
  if (vectorNode.strokeWeight === figma.mixed) {
    ...
    if (vectorNode.strokeWeight > 0)
      idealized.strokeWeight = vectorNode.strokeWeight;
  }
  ...
  try {
    ...
    idealized.vectorNetwork = JSON.parse(
      JSON.stringify(vectorNode.vectorNetwork)
    );
  } catch (error) {
    notifyError(`SVG failed to export: ${error}`);
...
export async function extractWorldviewNode(
  sceneNode: SceneNode
): Promise<WorldviewNode | null> {
  switch (sceneNode.type) {
    case 'FRAME':
    case 'COMPONENT':
    case 'COMPONENT_SET':
...
function resizeFrameToFitContent(
  frameNode: FrameNode | InstanceNode | ComponentNode | GroupNode
): void {
